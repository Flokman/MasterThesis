...
def load_data(path, to_shuffle):
    ...
    for lab in y_train:
        label_count[lab] += 1
    print("label count training: ", label_count)    
    ...
    for lab in y_val:
        label_count[lab] += 1
    print("label count validation: ", label_count)
    label_count = [0] * NUM_CLASSES
    for lab in y_test:
        label_count[lab] += 1
    print("label count test: ", label_count)
...
def main():
    ...
    results = MCBN_model.evaluate(x_test, y_test, batch_size=BATCH_SIZE)
